...
var letterScores = make(map[string]*[5]int)
...
func main() {
    ...
    flag.StringVar(&currGuess, "s", "cares", "Specify starting word. Default is 'cares'.")
...
func processWordFile() {
    ...
    for i:=0; scanner.Scan(); i++ {
        ...
        for i, c := range words[i] {
            letter := string(c)
            _, mapContainsLetter := letterScores[letter]
            if mapContainsLetter {
                letterScores[letter][i] += 1
            } else {
                letterScores[letter] = &[5]int{0, 0, 0, 0, 0}
                letterScores[letter][i] = 1
            }
        }
    }
...
// based on the frequency and position of the letters 
// and on the variety of letters in the word
